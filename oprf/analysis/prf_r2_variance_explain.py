...
###############################################----------------------New R2 approach---------------------------------############################
    # @classmethod
    # def get_r2_new_method(cls, Y_signals_gpu, O_gpu, epsilon_results_cpu):
    #     # Y*t_Y*
    #     Y_star_gpu = cp.dot(O_gpu, Y_signals_gpu)
    #     Y_star_Y_star_inverse_gpu = ((Y_star_gpu ** 2).sum(axis=0))** (-1)

    #     # Yt_Y
    #     Yt_Y_gpu = ((Y_signals_gpu ** 2).sum(axis=0)) * Y_star_Y_star_inverse_gpu
    #     # Yt_Y_gpu = cp.dot(Y_signals_gpu.T, Y_signals_gpu) * Y_star_Y_star_inverse_gpu

    #     # €
    #     e_term = cp.asarray(epsilon_results_cpu) * Y_star_Y_star_inverse_gpu

    #     # Yt_RRt_Y
    #     R_Rt_gpu = cp.eye(O_gpu.shape[0]) - O_gpu
    #     Yt_RRt = (Y_signals_gpu.T @ R_Rt_gpu) # .sum(axis=0))
    #     Yt_RRt_Y_gpu = ((Yt_RRt.T  * Y_signals_gpu).sum(axis=0)) * Y_star_Y_star_inverse_gpu  # each row would contain a scalar value for a given single y-signal
    #     # Yt_RRt_Y_gpu = (cp.dot(cp.dot(Y_signals_gpu.T , R_Rt_gpu), Y_signals_gpu)) * Y_star_Y_star_inverse_gpu  # each row would contain a scalar value for a given single y-signal

    #     r2 = 1 - (Yt_Y_gpu +  e_term + Yt_RRt_Y_gpu)
    ...
    # def get_r2_new_method(cls, Y_signals_gpu, O_gpu, epsilon_results_cpu):        
    #     # Yt_Y
    #     Yt_Y_gpu = ((Y_signals_gpu ** 2).sum(axis=0))
        
    #     # €
    #     e_term = cp.asarray(epsilon_results_cpu)

    #     # Yt_RRt_Y
    #     R_Rt_gpu = cp.eye(O_gpu.shape[0]) - O_gpu
    #     Yt_RRt = (Y_signals_gpu.T @ R_Rt_gpu) # .sum(axis=0))
    #     Yt_RRt_Y_gpu = ((Yt_RRt.T  * Y_signals_gpu).sum(axis=0))  # each row would contain a scalar value for a given single y-signal
    ...
    #     r2 = 1 - ((Yt_Y_gpu +  e_term + Yt_RRt_Y_gpu) * Y_star_Y_star_inverse_gpu)
    ...
    @classmethod
    def get_r2_num_den_method_with_epsilon_as_yTs(cls, Y_signals_gpu, O_gpu, refined_matching_results, x_range_cpu, y_range_cpu, stimulus):    
        stim_height, stim_width, stim_frames = stimulus.resampled_hrf_convolved_data.shape
        numerators_gpu , denominators_gpu = cls.get_r2_numerator_denominator_terms(Y_signals_gpu, O_gpu, refined_matching_results, x_range_cpu, y_range_cpu, stimulus, stim_height, stim_width)
        r2_results_gpu = 1 - (numerators_gpu / denominators_gpu)

        return cp.asnumpy(r2_results_gpu)   

    @classmethod # R2 numerator = (yty - e - yt_RRt_y)    
    def get_r2_numerator_denominator_terms(cls, Y_signals_gpu, O_gpu, refined_matching_results, x_range_cpu, y_range_cpu, stimulus, stim_height, stim_width):  
        num_y_signals = len(refined_matching_results)
