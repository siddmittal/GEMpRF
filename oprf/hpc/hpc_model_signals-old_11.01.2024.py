...
#         # sigma_array_gpu = cp.asarray(sigma_values_arr)
...
#     def _compute_timecourse_batches_not_workimg(self, kernel, x_range_gpu, y_range_gpu, stimulus_data_columnmajor_gpu):
#         if self.block_dim is None or self.grid_dim is None:
#             self.set_kernel_config()
...
#         # NOTE: compute "ASSIGNED" vs. "POSSIBLE" batch sizes
#         # Launch Gaussian related kernels
#         # compute all possible gaussian curves for each sigma value
#         num_gpus = 4 # NOTE: sigma batch can be decided based on the number of GPUs....then we know which sigma-batch is allocated on which GPU
#         total_sigmas = len(self.search_space_sigma_range_gpu)
#         per_gpu_assigned_sigma_batch_size = int(total_sigmas / num_gpus)
#         per_gpu_batch_required_mem = gpu_utils.gpu_mem_required_in_gb( num_elements= (self.model_nRows * self.model_nCols * per_gpu_assigned_sigma_batch_size * single_timecourse_length) * (1024 ** 3)) # bytes
#         single_sigma_grid_slice_required_bytes = gpu_utils.gpu_mem_required_in_gb( num_elements= (self.model_nRows * self.model_nCols) * (self.stim_width * self.stim_height)) * (1024 ** 3) # bytes
#         gpu_idx = 0
#         for sigma_batch_idx in range(0, total_sigmas, per_gpu_assigned_sigma_batch_size):
...
#                 if per_gpu_batch_required_mem > available_bytes:
#                     raise MemoryError(f"Not enough GPU memory available on device-{gpu_idx} to store the assigned batch of model signals.\nAvailable (Gigabytes) = {available_bytes / (1024 ** 3)}, Required (Gigabytes) = {(per_gpu_batch_required_mem * 2) / (1024 ** 3)}.\nGEM-pRF cannot compute further model signals !!!")
...
#                 selected_gpu_possible_sigma_batch_size = per_gpu_assigned_sigma_batch_size if per_gpu_assigned_sigma_batch_size <= possible_sigma_batch_size else possible_sigma_batch_size
#                 if gpu_idx == 0:
#                     signal_rowmajor_batch_current_gpu = cp.zeros((self.model_nRows * self.model_nCols * per_gpu_assigned_sigma_batch_size, single_timecourse_length), dtype=cp.float64)
#                     for sigma_chunk_idx in range(0, per_gpu_assigned_sigma_batch_size, selected_gpu_possible_sigma_batch_size): # (0, total_sigma, batch_size)
#                         from_sigma_idx = (sigma_batch_idx * per_gpu_assigned_sigma_batch_size) +  sigma_chunk_idx
#                         to_sigma_idx =  (sigma_batch_idx * per_gpu_assigned_sigma_batch_size) +  (sigma_chunk_idx + selected_gpu_possible_sigma_batch_size)
#                         sigma_values_arr_gpu = self.search_space_sigma_range_gpu[from_sigma_idx : to_sigma_idx]
#                         chunk_signal_rowmajor_batch_gpu = self._compute_timecourses_chunk(kernel, x_range_gpu, y_range_gpu, sigma_values_arr_gpu, selected_gpu_possible_sigma_batch_size, stimulus_data_columnmajor_gpu, gpu_idx)
#                         signal_rowmajor_batch_current_gpu[sigma_chunk_idx * num_signals_per_sigma: (sigma_chunk_idx + selected_gpu_possible_sigma_batch_size) * num_signals_per_sigma, :] = chunk_signal_rowmajor_batch_gpu
#                 else:
#                     sigma_values_arr_gpu = self.search_space_sigma_range_gpu[sigma_batch_idx : sigma_batch_idx + per_gpu_assigned_sigma_batch_size]
#                     signal_rowmajor_batch_current_gpu = self._compute_timecourses_chunk(kernel, x_range_gpu, y_range_gpu, sigma_values_arr_gpu, per_gpu_assigned_sigma_batch_size, stimulus_data_columnmajor_gpu, gpu_idx)

#             # # copy the results back to gpu-0
#             # if gpu_idx != 0:
#             #     with cp.cuda.Device(0):
#             #         signal_rowmajor_batch_gpu = cp.array(signal_rowmajor_batch_current_gpu)
#             # else:
#             #     signal_rowmajor_batch_gpu = signal_rowmajor_batch_current_gpu

#             # # update the computed batch to the final array
#             # all_model_signals_rowmajor_gpu[sigma_batch_idx * num_signals_per_sigma: (sigma_batch_idx + per_gpu_assigned_sigma_batch_size) * num_signals_per_sigma, :] = signal_rowmajor_batch_gpu
#             gpu_idx = gpu_idx + 1 # NOTE: later on, we will specify the GPU numbers in the config, and read the value from there
...
#     #     # result bacthes maybe distributed across multiple GPUs
#     #     result_batches = []
...
#     #     single_sigma_grid_slice_required_bytes = gpu_utils.gpu_mem_required_in_gb( num_elements= (self.model_nRows * self.model_nCols) * (self.stim_width * self.stim_height))
#     #     gpu_idx = 0
#     #     for sigma_batch_idx in range(0, total_sigmas, per_gpu_assigned_sigma_batch_size):
#     #         signal_rowmajor_batch_current_gpu = None
...
#     #         with cp.cuda.Device(gpu_idx):
#     #             # signal_rowmajor_batch_current_gpu = cp.zeros((self.model_nRows * self.model_nCols * per_gpu_assigned_sigma_batch_size, single_timecourse_length), dtype=cp.float64)
#     #             stimulus_data_current_device = None
#     #             if gpu_idx == 0:
#     #                 stimulus_data_current_device = stimulus_data_columnmajor_gpu
#     #             else:
#     #                 stimulus_data_current_device = cp.array(stimulus_data_columnmajor_gpu)
...
#     #                     to_sigma_idx =  (sigma_batch_idx * per_gpu_assigned_sigma_batch_size) +  (sigma_chunk_idx + selected_gpu_possible_sigma_batch_size)
#     #                     sigma_values_arr_gpu = self.search_space_sigma_range_gpu[from_sigma_idx : to_sigma_idx]
#     #                     chunk_signal_rowmajor_batch_gpu = self._compute_timecourses_chunk(kernel, x_range_gpu, y_range_gpu, sigma_values_arr_gpu, selected_gpu_possible_sigma_batch_size, stimulus_data_current_device, gpu_idx)
#     #                     signal_rowmajor_batch_current_gpu[sigma_chunk_idx * num_signals_per_sigma: (sigma_chunk_idx + selected_gpu_possible_sigma_batch_size) * num_signals_per_sigma, :] = chunk_signal_rowmajor_batch_gpu
...
#     #     return result_batches # all_model_signals_rowmajor_gpu
...
#             signal_rowmajor_batch_current_gpu = None
#             # available_bytes = gpu_utils.device_available_mem_bytes(device_id=gpu_idx)
#             # num_possible_sigmas_to_process = int(available_bytes / single_sigma_grid_slice_required_bytes * 2)
#             # num_possible_sigmas_to_process = per_gpu_assigned_sigma_batch_size if per_gpu_assigned_sigma_batch_size <= num_possible_sigmas_to_process else num_possible_sigmas_to_process                
...
#                 if possible_sigma_batch_size < 1:
#                     raise ValueError(f"Not enough GPU memory available on device-{gpu_idx}.\nAvailable (Gigabytes) = {available_bytes / (1024 ** 3)}, Required (Gigabytes) = {(single_sigma_grid_slice_required_bytes * 2) / (1024 ** 3)}.\nGEM-pRF cannot compute further model signals !!!")
#                 selected_gpu_possible_sigma_chunk_size = 5 #per_gpu_assigned_sigma_batch_size if per_gpu_assigned_sigma_batch_size <= possible_sigma_batch_size else possible_sigma_batch_size                
...
#                         from_sigma_idx = (sigma_batch_idx * per_gpu_assigned_sigma_batch_size) +  sigma_chunk_idx  
#                         max_chunk_idx = (sigma_batch_idx + 1)*per_gpu_assigned_sigma_batch_size
#                         if (sigma_chunk_idx + selected_gpu_possible_sigma_chunk_size) > max_chunk_idx:
#                             chunk_size = max_chunk_idx - from_sigma_idx
#                         else:
#                             chunk_size = selected_gpu_possible_sigma_chunk_size
#                         to_sigma_idx =  (sigma_batch_idx * per_gpu_assigned_sigma_batch_size) +  (sigma_chunk_idx + chunk_size)                   
#                         sigma_values_arr_gpu = self.search_space_sigma_range_gpu[from_sigma_idx : to_sigma_idx]
#                         chunk_signal_rowmajor_batch_gpu = self._compute_timecourses_chunk(kernel, x_range_gpu, y_range_gpu, sigma_values_arr_gpu, chunk_size, stimulus_data_current_device, gpu_idx)            
...
#                 else:                     
#                     signal_rowmajor_batch_current_gpu = self._compute_timecourses_chunk(kernel, x_range_gpu, y_range_gpu, sigma_values_arr_gpu, per_gpu_assigned_sigma_batch_size, stimulus_data_current_device, gpu_idx)
...
#         #return all_model_signals_rowmajor_gpu, result_batches
