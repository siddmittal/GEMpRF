...
# class RefineFit:
#     @classmethod
#     def get_refined_fit_results(cls, prf_space : PRFSpace, num_Y_signals, best_fit_proj_cpu, arr_2d_location_inv_M_cpu, e_full, de_dtheta_list_cpu, search_space_rows, search_space_cols, search_space_frames
#                                 , search_space_xx
#                                 , search_space_yy
...
#             # # vec = np.vstack((e_vec))
#             # # num_theta_params = len(de_dtheta_list_cpu)
#             # # for theta in range(num_theta_params):
#             # #     vec = np.vstack((vec, de_dtheta_list_cpu[theta][y_idx, block_flat_indices].reshape(-1, 1)))
#             # # vec = vec.ravel(order = 'F')          
#             # wrong_vec = np.hstack((e_vec.squeeze(), np.array(de_dtheta_list_cpu).squeeze()[:, block_flat_indices].squeeze().T.flatten()))

#             vec = np.vstack((e_vec.squeeze())).squeeze()
#             for theta in range(3):
#                 # vec = np.vstack((vec, (np.array(de_dtheta_list_cpu).squeeze()[:, block_flat_indices].squeeze().T)[:, theta]))	                                                
#                 # vec = np.vstack((vec, (np.array(de_dtheta_list_cpu).squeeze()[theta, y_idx, block_flat_indices].squeeze().T)[:, theta]))	                                                
#                 #vec = np.vstack((vec, (de_dtheta_list_cpu[theta, y_idx, block_flat_indices].squeeze().T)))
...
#             # neighbours_gaussian_args = prf_points.multi_dim_points_cpu[block_flat_indices]
#             # neighbours_gaussian_args = (neighbours_gaussian_args.squeeze()).reshape((27, 3))
#             # neighbours_gaussian_args[:, [0, 1]] = neighbours_gaussian_args[:, [1, 0]] ## to make (col, row) i.e. (x, y) convention

#             # num_neighbours = len(neighbours_gaussian_args)                                                     
#             # M = np.zeros((num_neighbours * num_linear_equations, num_unknown_coefficients), dtype=float)
#             # for i in  range(len(neighbours_gaussian_args)):
#             #     cofX = neighbours_gaussian_args[i]
#             #     # cofX[[0, 1]] = cofX[[1, 0]] # NOTE: not needed at the moment, as for debugging, I have purposely set mu_x and mu_y values to be equal i.e. (2.16, 2.16)
#             #     # cofX = (cofX.T).flatten()            
#             #     x = np.array(
#             #         [[cofX[0] ** 2  ,   cofX[1] ** 2    , cofX[2] ** 2  ,   2. * cofX[0] * cofX[1]      ,   2. * cofX[0] * cofX[2]      ,    2. * cofX[1] * cofX[2]     ,   cofX[0]     ,   cofX[1]     ,   cofX[2] ,   1],
#             #             [2. * cofX[0]  ,   0               , 0             ,   2. * cofX[1]                ,   2. * cofX[2]                ,    0                          ,   1           ,   0           ,   0       ,   0],
#             #             [0             ,   2. * cofX[1]    , 0             ,   2. * cofX[0]                ,   0                           ,    2. * cofX[2]               ,   0           ,   1           ,   0       ,   0],
#             #             [0             ,   0               , 2. * cofX[2]  ,   0                           ,   2. * cofX[0]                ,    2. * cofX[1]               ,   0           ,   0           ,   1       ,   0]]
#             #             )
#             #     M[i * num_linear_equations : (i * num_linear_equations) + num_linear_equations, :] = x
#             # test_MpInv = np.linalg.pinv(M)             


#             # compute the coefficients
#             coefficients = MpInv@vec
#             # coefficients = test_MpInv@vec
...
#                     coarse_gradients = [e_full[y_idx, best_s_idx], de_dtheta_list_cpu[0][y_idx, best_s_idx], de_dtheta_list_cpu[1][y_idx, best_s_idx], de_dtheta_list_cpu[1][y_idx, best_s_idx]]
#                     dummy2 = 0
