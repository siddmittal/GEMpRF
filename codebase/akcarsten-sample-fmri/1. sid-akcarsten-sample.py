...
local_data_directory = os.path.abspath(os.path.join(script_directory, "../../local-extracted-datasets"))
...
local_fmri_data_dir = os.path.join(local_data_directory, 'fMRI_data')
if os.path.exists(local_fmri_data_dir) == False:
    os.mkdir(local_fmri_data_dir)
    
open(os.path.join(local_fmri_data_dir, 'data.zip'), 'wb').write(data.content)
# Un-zip the file
zip_ref = zipfile.ZipFile(os.path.join(local_fmri_data_dir, 'data.zip'), 'r')
zip_ref.extractall(local_fmri_data_dir)
zip_ref.close()
...
data_path = os.path.join(local_fmri_data_dir, 'sM00223/') 
...
data_path = os.path.join(local_fmri_data_dir, 'fM00223/')
files = os.listdir(data_path)
...
csv_data_dir = os.path.join(local_fmri_data_dir, 'csv_data')
if os.path.exists(csv_data_dir) == False:
...
for export in data_all:
    ...
    save_path = os.path.join(csv_data_dir, '{}'.format(save_file))
