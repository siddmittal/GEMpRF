...
def gpu_fitting(O_gpu, arr_2d_location_inv_M_cpu, Y_signals_gpu, Y_signals_cpu, S_prime_columnmajor_gpu_batches, dS_prime_dx_columnmajor_gpu_batches, dS_prime_dy_columnmajor_gpu_batches, dS_prime_dsigma_columnmajor_gpu_batches):
    ...
    refined_results_list, Fex_results = RefineFit.get_refined_fit_results(Y_signals_cpu.shape[1]
                                                        , best_fit_proj_cpu
    ...
    coarse_estimated_3d_positions = tuple(coarse_estimated_3d_positions_arr[:, i] for i in range(coarse_estimated_3d_positions_arr.shape[1])) #(coarse_estimated_3d_positions_arr[:, 0], coarse_estimated_3d_positions_arr[:, 1], coarse_estimated_3d_positions_arr[:, 2])
    ...
    return refined_results_arr.tolist(), Fex_results # refined_results_list
...
def main():
    ...
    for data_idx in range(len(measured_data)):
        ...
        for current_batch_idx in range(0, total_y_signals, batch_size):
            ...
            #r2_results = R2.get_r2_new_method_with_epsilon_as_yTs(Y_signals_batch_gpu, O_gpu, refined_matching_results, x_range_cpu, y_range_cpu, stimulus, stim_height, stim_width, stim_frames)
