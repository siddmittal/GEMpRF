...
def gpu_fitting(O_gpu, arr_2d_location_inv_M_cpu, Y_signals_gpu, Y_signals_cpu, S_prime_columnmajor_gpu_batches, dS_prime_dx_columnmajor_gpu_batches, dS_prime_dy_columnmajor_gpu_batches, dS_prime_dsigma_columnmajor_gpu_batches):
    ...
    refined_results,  results_line_intersection \
    ...
    return refined_results, results_line_intersection, e_cpu, de_dx_full_cpu, de_dy_full_cpu, de_dsigma_full_cpu
...
def main():
    ...
    # Test errors - quadratic approximation
    line_refine_e_full_cpu, line_refine_de_dx_full_cpu, line_refine_de_dy_full_cpu, line_refine_de_dsigma_full_cpu = RefineFit.get_all_debug_info_error_terms_after_refinement(refined_matching_results = line_refined_matching_results,
                                                                                                  Y_signals_gpu = Y_signals_gpu,
                                                                                                  O_gpu = O_gpu,
                                                                                                  stimulus = stimulus,
                                                                                                  stim_height = stim_height,
    ...
    line_r2_results = R2.get_r2_num_den_method_with_epsilon_as_yTs(Y_signals_gpu, O_gpu, line_refined_matching_results, x_range_cpu, y_range_cpu, stimulus)
    ...
    df = pd.DataFrame(columns = ["CoarseEstimations", "QuadRefinedEstimations", "LineRefinedEstimations", "CoarseErrors", "QuadErrors", "LineErrors", "QuadR2", "LineR2", "MatchedModelSignal", "YSignal"])
    ...
    for y_idx in range(0, Y_signals_cpu.shape[1]):
        ...
        #...compute Line-REFINED de/dx, de/dy and de/dsigma vectors#
        line_refine_e_cpu = line_refine_e_full_cpu[y_idx, best_refined_idx]
        line_refine_de_dx_cpu = line_refine_de_dx_full_cpu[y_idx, best_refined_idx]
        line_refine_de_dy_cpu = line_refine_de_dy_full_cpu[y_idx, best_refined_idx]
        line_refine_de_dsigma_cpu = line_refine_de_dsigma_full_cpu[y_idx, best_refined_idx]
        line_refined_errors = [line_refine_e_cpu, line_refine_de_dx_cpu, line_refine_de_dy_cpu, line_refine_de_dsigma_cpu]
        ...
        # line - refined estimations
        line_refined_matching_results_arr = np.array(line_refined_matching_results)
        line_refined_estimations = line_refined_matching_results_arr[y_idx]

        # Append Dict as row to DataFrame
        df.loc[y_idx] = [coarse_estimations, quad_refined_estimations, line_refined_estimations, coarse_errors, quad_refined_errors, line_refined_errors, quad_r2_results[y_idx], line_r2_results[y_idx], matched_model_signal, y_signal]
