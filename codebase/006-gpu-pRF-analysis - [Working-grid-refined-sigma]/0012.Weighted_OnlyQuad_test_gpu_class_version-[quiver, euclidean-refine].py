...
def main():
    ...
    # get weighted M-inverse matrix  
    #..compute sum of squares for each signal  
    Wx = 1./cp.asnumpy(cp.sum(cp.square(dS_prime_dx_columnmajor_gpu[0]), axis=0))
    Wy = 1./cp.asnumpy(cp.sum(cp.square(dS_prime_dy_columnmajor_gpu[0]), axis=0))
    Wsigma = 1./cp.asnumpy(cp.sum(cp.square(dS_prime_dsigma_columnmajor_gpu[0]), axis=0))
    W = np.column_stack((np.ones(len(dS_prime_dy_columnmajor_gpu[0].T), dtype=np.float64), Wx, Wy, Wsigma))
    arr_2d_location_inv_M_cpu = ParallelComputedCoefficientMatrix.Wrapper_Weighted_Grids2MpInv_numba(mu_X_grid, mu_Y_grid, sigma_grid, W) ##<<<<--------Compute pre-define matrix M
