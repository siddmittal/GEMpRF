...
def gpu_fitting(O_gpu, arr_2d_location_inv_M_cpu, Y_signals_gpu, Y_signals_cpu, S_prime_columnmajor_gpu_batches, dS_prime_dx_columnmajor_gpu_batches, dS_prime_dy_columnmajor_gpu_batches, dS_prime_dsigma_columnmajor_gpu_batches):
    ...
    best_fit_proj_cpu, coarse_e_cpu, de_dx_full_cpu, de_dy_full_cpu, de_dsigma_full_cpu = GridFit.get_error_terms(isResultOnGPU=False,
                                                                                                           O_gpu=O_gpu,
    ...
    # refined_results,  Fex_results \
    refined_results_list,  results_line_intersection \
    = RefineFit.get_refined_fit_results(Y_signals_cpu.shape[1]
                                                        , best_fit_proj_cpu
    ...
    # Keep the coarse estimaitons if the errors are getting worse after refinement
    # ...get error terms after refinement (to verfiy improvement)
    refined_e_cpu = RefineFit.get_error_terms_after_refinement(refined_matching_results = refined_results_list, 
                                               Y_signals_gpu=Y_signals_gpu,
                                              O_gpu=O_gpu,
                                              stimulus=stimulus,
                                              stim_height=stim_height,
                                              stim_width=stim_width,
                                              x_range_gpu=x_range_gpu,
                                              y_range_gpu=y_range_gpu)
    ...
    # ...get 3D indices of the coarse estimations for the worsened-error indices
    coarse_estimated_3d_positions_arr = MatrixOps.flatIdx2ThreeDimIndices(best_fit_proj_cpu[worsened_error_y_signal_indices], search_space_rows, search_space_cols, search_space_frames)
    coarse_estimated_3d_positions = tuple(coarse_estimated_3d_positions_arr[:, i] for i in range(coarse_estimated_3d_positions_arr.shape[1])) #(coarse_estimated_3d_positions_arr[:, 0],
    ss = SearchSpace.get_instance()
    coarse_estimations = np.column_stack((ss.meshgrid_X[coarse_estimated_3d_positions], ss.meshgrid_Y[coarse_estimated_3d_positions], ss.meshgrid_Sigma[coarse_estimated_3d_positions]))

    # ...replace refined estimations with the coarse estimations
    refined_results_arr = np.array(refined_results_list)
    refined_results_arr[worsened_error_y_signal_indices.reshape(-1)] = coarse_estimations    

    # return refined_results, Fex_results, e_cpu, de_dx_full_cpu, de_dy_full_cpu, de_dsigma_full_cpu
    return refined_results_arr.tolist(), results_line_intersection, coarse_e_cpu, de_dx_full_cpu, de_dy_full_cpu, de_dsigma_full_cpu
...
def main():
    ...
    MpInv_thread = threading.Thread(target=execute_Grids2MpInv, args=(ss.meshgrid_X, ss.meshgrid_Y, ss.meshgrid_Sigma, result_queue))
    MpInv_thread.start()
    ...
    # refine fitting
    # quad_refined_matching_results, line_refined_matching_results, e_cpu, de_dx_full_cpu, de_dy_full_cpu, de_dsigma_full_cpu = gpu_fitting(O_gpu, arr_2d_location_inv_M_cpu, Y_signals_gpu, Y_signals_cpu, S_prime_columnmajor_gpu, dS_prime_dx_columnmajor_gpu, dS_prime_dy_columnmajor_gpu, dS_prime_dsigma_columnmajor_gpu)
    quad_refined_matching_results, fex_errors_results, e_cpu, de_dx_full_cpu, de_dy_full_cpu, de_dsigma_full_cpu = gpu_fitting(O_gpu, arr_2d_location_inv_M_cpu, Y_signals_gpu, Y_signals_cpu, S_prime_columnmajor_gpu, dS_prime_dx_columnmajor_gpu, dS_prime_dy_columnmajor_gpu, dS_prime_dsigma_columnmajor_gpu)


    # Test errors - quadratic approximation
    quad_refine_e_full_cpu, quad_refine_de_dx_full_cpu, quad_refine_de_dy_full_cpu, quad_refine_de_dsigma_full_cpu = RefineFit.get_all_debug_info_error_terms_after_refinement(refined_matching_results = quad_refined_matching_results,
                                                                                                  Y_signals_gpu = Y_signals_gpu,
                                                                                                  O_gpu = O_gpu,
                                                                                                  stimulus = stimulus,
                                                                                                  stim_height = stim_height,
    ...
    quad_r2_results = R2.get_r2_num_den_method_with_epsilon_as_yTs(Y_signals_gpu, O_gpu, quad_refined_matching_results, x_range_cpu, y_range_cpu, stimulus)    

    # create dataframe
    # df = pd.DataFrame(columns = ["CoarseEsitmations", "RefinedEstimations", "CoarseGradients", "RefinedGradients"])
    # df = pd.DataFrame(columns = ["CoarseEstimations", "QuadRefinedEstimations", "LineRefinedEstimations", "CoarseGradients", "QuadRefinedGradients", "LineRefinedGradients"])
    df = pd.DataFrame(columns = ["CoarseEstimations", "QuadRefinedEstimations", "CoarseErrors", "QuadErrors", "FexError", "QuadR2", "MatchedModelSignal", "YSignal"])
    # df = pd.DataFrame(columns = ["type", "param", "value"])51*51
    df_idx = 0
    best_fit_proj_cpu = np.nanargmax(e_cpu, axis=1)
    # best_fit_proj_cpu = np.nanargmax(e_cpu[:, 51*51:-51*51], axis=1) + 51*51
    # for y_idx in range(0, Y_signals_cpu.shape[1], 10):
    for y_idx in range(0, Y_signals_cpu.shape[1]):
        ...
        coarse_errors = [e_vec_cpu, de_dx_vec_cpu, de_dy_vec_cpu, de_dsigma_vec_cpu]

        best_refined_idx = y_idx

        y_signal = None
        matched_model_signal = None
        if (y_idx == 310 or y_idx == 920 or y_idx == 1200 or y_idx == 5520):
            y_signal = Y_signals_cpu[:, y_idx]
            matched_model_signal = cp.asnumpy((S_prime_columnmajor_gpu[0])[:, best_s_idx])

        #...compute QUAD-REFINED de/dx, de/dy and de/dsigma vectors#
        quad_refine_e_cpu = quad_refine_e_full_cpu[y_idx, best_refined_idx]
        quad_refine_de_dx_cpu = quad_refine_de_dx_full_cpu[y_idx, best_refined_idx]
        quad_refine_de_dy_cpu = quad_refine_de_dy_full_cpu[y_idx, best_refined_idx]
        quad_refine_de_dsigma_cpu = quad_refine_de_dsigma_full_cpu[y_idx, best_refined_idx]
        quad_refined_errors = [quad_refine_e_cpu, quad_refine_de_dx_cpu, quad_refine_de_dy_cpu, quad_refine_de_dsigma_cpu]
        ...
        x = ss.meshgrid_X[x_coarse_idx, y_coarse_idx, sigma_coarse_idx]
        y = ss.meshgrid_Y[x_coarse_idx, y_coarse_idx, sigma_coarse_idx]
        sigma = search_space_sigma_range[sigma_coarse_idx]
        coarse_estimations = [x, y, sigma]

        # quad - refined estimations
        quad_refined_matching_results_arr = np.array(quad_refined_matching_results)
        quad_refined_estimations = quad_refined_matching_results_arr[y_idx]

        # Append Dict as row to DataFrame
        # df.loc[y_idx] = [coarse_estimations, quad_refined_estimations, coarse_errors, quad_refined_errors, quad_r2_results[y_idx], matched_model_signal, y_signal]
        df.loc[y_idx] = [coarse_estimations, quad_refined_estimations, coarse_errors, quad_refined_errors, fex_errors_results[y_idx] , quad_r2_results[y_idx], matched_model_signal, y_signal]

    result_filepath = cfg.results['basepath'] + (str(str(datetime.date.today()) + '_') if cfg.results['prepend_date'] == "True" else '') + 'gradients-analysis' + cfg.results['custom_filename_postfix'] + '.xlsx'
    df.to_excel(result_filepath, index=False)
    # df.to_excel(os.path.abspath(os.path.join(os.path.dirname( __file__ ), '..', '..', '..', '..', 'gem-results/gradients-test/30.01.2024-refinement-[signals_simga-14c_boundary-1_quad-vs-line-EMPIRICAL]_fx-51x51x8.xlsx')), index=False)  # Set index=False if you don't want to write row indices
    print('done')
