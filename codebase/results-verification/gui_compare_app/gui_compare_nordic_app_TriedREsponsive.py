...
###################################------------------------------------------------GUI-------------------------------------------------############################################
###################################------------------------------------------------GUI-------------------------------------------------############################################
def update_plots(filtered_data_standard, filtered_data_nordic, covmap_path_standard, covmap_path_nordic, fmri_measured_timecourse_data_standard, fmri_measured_timecourse_data_nordic, voxel_number):
    try:
        ...
        ecc_scatter_plot.set_title('Eccentricity Scatter Plot')
        ...
        sigma_scatter_plot.set_title('pRF Size Scatter Plot')
        ...
        sigma_hist_plot.set_title('pRF Size Histogram')
...
def load_data():
    ...
    update_plots(filtered_voxel_esitmations_data_standard, filtered_voxel_esitmations_data_nordic, covmap_path_standard, covmap_path_nordic, fmri_measured_data_timecourse_standard, fmri_measured_data_timecourse_nordic, voxel_number)
    ...
    r2_label.config(text=f"R2: {filtered_voxel_esitmations_data_standard.iloc[0]['R2']}")
...
# # sub_listbox.grid(row=0, column=0)
# # ses_listbox.grid(row=0, column=1)
# # run_listbox.grid(row=0, column=2)


# # # Label for displaying Centerx0, Centery0
# # r2_label = tk.Label(root, text="R2")
# # r2_label.grid(row=0, column=3, columnspan=8)
...
# sub_listbox.grid(row=0, column=0, sticky="nsew")
# ses_listbox.grid(row=0, column=1, sticky="nsew")
# run_listbox.grid(row=0, column=2, sticky="nsew")
...
# # Label for displaying R2 value
# r2_label = tk.Label(root, text="R2")
# r2_label.grid(row=0, column=3, columnspan=8, sticky="ew")

# # Slider and Entry for Voxel Number
# voxel_slider = tk.Scale(root, from_=0, to=20000, orient=tk.HORIZONTAL, label="Voxel Number", length=600)
# voxel_slider.bind("<ButtonRelease-1>", lambda event: update_voxel_entry(voxel_slider.get()))
# voxel_slider.grid(row=2, column=0, columnspan=20, sticky="ew")

# voxel_entry = tk.Entry(root)
# voxel_entry.grid(row=3, column=7, sticky="ew")
# voxel_entry.bind("<Return>", update_voxel_slider)
# voxel_slider.set(15)  # Set initial value for the slider
# update_voxel_entry(voxel_slider.get())  # Sync the entry with the slider
...
# # Button to load data based on inputs
# load_button = tk.Button(root, text="Load Data", command=load_data)
# load_button.grid(row=3, column=0, columnspan=6, sticky="ew")

# # Canvas and plot area configurations (example for one plot)
# ecc_scatter_figure = Figure(figsize=(5, 5))
# ecc_scatter_canvas = FigureCanvasTkAgg(ecc_scatter_figure, root)
# ecc_scatter_canvas.get_tk_widget().grid(row=4, column=0, columnspan=10, sticky="nsew")

# # Make the plotting area resizable
# root.grid_rowconfigure(4, weight=10)  # Adjust weight as needed
...
# # Configure the root window's grid layout to be responsive
# root.grid_rowconfigure(5, weight=1)
# root.grid_rowconfigure(11, weight=1)
# root.grid_columnconfigure([4, 8, 12], weight=1)

# # Eccentricity Histogram plot area
# ecc_hist_figure = Figure(figsize=(5, 5))
# ecc_hist_canvas = FigureCanvasTkAgg(ecc_hist_figure, master=root)
# ecc_hist_canvas.get_tk_widget().grid(row=5, column=4, columnspan=4, sticky="nsew")

# # Sigma Scatter plot area
# sigma_scatter_figure = Figure(figsize=(5, 4), dpi=100)
# sigma_scatter_canvas = FigureCanvasTkAgg(sigma_scatter_figure, master=root)
# sigma_scatter_canvas.get_tk_widget().grid(row=5, column=8, columnspan=4, sticky="nsew")

# # Sigma Histogram plot area
# sigma_hist_figure = Figure(figsize=(5, 4), dpi=100)
# sigma_hist_canvas = FigureCanvasTkAgg(sigma_hist_figure, master=root)
# sigma_hist_canvas.get_tk_widget().grid(row=5, column=12, columnspan=4, sticky="nsew")

# # Covmap plot area - standard
# covmap_figure_standard = Figure(figsize=(5, 4), dpi=100)
# covmap_canvas_standard = FigureCanvasTkAgg(covmap_figure_standard, master=root)
# covmap_canvas_standard.get_tk_widget().grid(row=11, column=0, columnspan=4, sticky="nsew")

# # Covmap plot area - nordic
# covmap_figure_nordic = Figure(figsize=(5, 4), dpi=100)
# covmap_canvas_nordic = FigureCanvasTkAgg(covmap_figure_nordic, master=root)
# covmap_canvas_nordic.get_tk_widget().grid(row=11, column=4, columnspan=4, sticky="nsew")

# # Timecourses plot area
# timecourses_figure = Figure(figsize=(10, 4), dpi=100)
# timecourses_canvas = FigureCanvasTkAgg(timecourses_figure, master=root)
# timecourses_canvas.get_tk_widget().grid(row=11, column=8, columnspan=8, sticky="nsew")
...
ecc_scatter_figure = Figure(figsize=(5, 5))
ecc_scatter_canvas = FigureCanvasTkAgg(ecc_scatter_figure, root)
ecc_scatter_canvas.get_tk_widget().grid(row=5, column=0, columnspan=4, sticky="nsew")
...
root.grid_rowconfigure(4, weight=10)  # More weight for plots
root.grid_rowconfigure(5, weight=1)
root.grid_rowconfigure(11, weight=1)
root.grid_columnconfigure(0, weight=1)
root.grid_columnconfigure(1, weight=1)
