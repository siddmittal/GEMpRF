...
ecc_scatter_figure = Figure(figsize=(3, 2.5), dpi=80)
ecc_scatter_canvas = FigureCanvasTkAgg(ecc_scatter_figure, master=root)
ecc_scatter_canvas.get_tk_widget().grid(row=5, column=0, columnspan=2)

# Eccentricity Histogram plot area
ecc_hist_figure = Figure(figsize=(3, 2.5), dpi=80)
ecc_hist_canvas = FigureCanvasTkAgg(ecc_hist_figure, master=root)
ecc_hist_canvas.get_tk_widget().grid(row=5, column=2, columnspan=2)

# Sigma Scatter plot area
sigma_scatter_figure = Figure(figsize=(3, 2.5), dpi=80)
sigma_scatter_canvas = FigureCanvasTkAgg(sigma_scatter_figure, master=root)
sigma_scatter_canvas.get_tk_widget().grid(row=5, column=4, columnspan=2)

# Sigma Histogram plot area
sigma_hist_figure = Figure(figsize=(3, 2.5), dpi=80)
sigma_hist_canvas = FigureCanvasTkAgg(sigma_hist_figure, master=root)
sigma_hist_canvas.get_tk_widget().grid(row=5, column=6, columnspan=2)

# covmap plot area - standard
covmap_figure_standard = Figure(figsize=(3, 2.5), dpi=80)
covmap_canvas_standard = FigureCanvasTkAgg(covmap_figure_standard, master=root)
covmap_canvas_standard.get_tk_widget().grid(row=11, column=0, columnspan=2)

# covmap plot area - nordic
covmap_figure_nordic = Figure(figsize=(3, 2.5), dpi=80)
covmap_canvas_nordic = FigureCanvasTkAgg(covmap_figure_nordic, master=root)
covmap_canvas_nordic.get_tk_widget().grid(row=11, column=2, columnspan=2)

# Timecourses plot area
timecourses_figure = Figure(figsize=(6, 2.5), dpi=80)
