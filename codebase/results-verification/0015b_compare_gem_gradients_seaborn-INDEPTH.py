...
line_refine_est = df3['LineRefinedEstimations']
coarse_errors = df3['CoarseErrors']
quad_errors = df3['QuadErrors']
line_errors = df3['LineErrors']
quad_r2 = df3['QuadR2'].to_numpy()
line_r2 = df3['LineR2'].to_numpy()
...
for idx in range(len(coarse_est)):
    ...
    line_refine_est_values = [float(val) for val in line_refine_est[idx][1:-1].split()]
    ...
    line_error_values = np.array([0, 0, 0, 0]) if (line_errors[idx]).find('nan') !=-1  else ast.literal_eval(line_errors[idx])
    ...
    line_refine_result.append(np.array(line_refine_est_values))

    coarse_errors_result.append(np.array(coarse_error_values))
    quad_errors_result.append(np.array(quad_error_values))
    line_errors_result.append(np.array(line_error_values))
...
line_errors = (np.array(line_errors_result))[:, 0]
...
ax1.plot(line_errors, label='Line Errors')
ax1.set_ylabel('Errors')
ax1.legend()

# Subplot 2: Differences among arrays
ax2.plot(coarse_errors - quad_errors, label='Coarse - Quad', color='red')
ax2.plot(coarse_errors - line_errors, label='Coarse - Line', color='blue')
ax2.plot(quad_errors - line_errors, label='Quad - Line', color='green')
...
ax2.hist(coarse_errors - line_errors, 60, label='Coarse - Line', color='blue')
ax3.hist(quad_errors - line_errors, 60, label='Quad - Line', color='green')
...
ax2.scatter(coarse_sigma, coarse_errors - line_errors)
ax3.scatter(coarse_sigma, quad_errors - line_errors)
