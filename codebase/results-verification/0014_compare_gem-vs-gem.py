...
def plot_all(title, gem_centerx0, oprf_centery0, gem_centery0, oprf_centerx0, gem_sigma, oprf_sigma, gem_r2, oprf_r2, Result_image_path):
    ...
    # Plot 1
    ax1 = axes[0, 0]
    ax1.scatter(gem_centery0, oprf_centery0, s=1)  # Swap axes
    ax1.set_xlabel(x_axis_label, fontsize=label_font_size)  # Swap labels
    ax1.set_ylabel(y_axis_name, fontsize=label_font_size)  # Swap labels
    ...
    # Plot 2
    ax2 = axes[0, 1]
    ax2.scatter(gem_centerx0, oprf_centerx0, s=1)  # Swap axes
    ax2.set_xlabel(x_axis_label, fontsize=label_font_size)  # Swap labels
    ax2.set_ylabel(y_axis_name, fontsize=label_font_size)  # Swap labels
    ...
    # Plot 3
    ax3 = axes[1, 0]
    ax3.scatter(gem_sigma, oprf_sigma, s=1)  # Swap axes
    ax3.set_xlabel(x_axis_label, fontsize=label_font_size)  # Swap labels
    ax3.set_ylabel(y_axis_name, fontsize=label_font_size)  # Swap labels
    ...
    # Plot 4
    ax4 = axes[1, 1]
    ax4.scatter(gem_r2, oprf_r2, s=1)  # Swap axes
    ax4.set_xlabel(x_axis_label, fontsize=label_font_size)  # Swap labels
    ax4.set_ylabel(y_axis_name, fontsize=label_font_size)  # Swap labels
    ...
    # ax5 = axes[2, 0]
    # with open("Y:/data/stimsim23/derivatives/prfresult/analysis-03/covMap/sub-sidtest/ses-001/gem_covmap.svg", 'rb') as svg_file:
    #     svg_content = svg_file.read()
    #     img = convert_svg_to_png(svg_content)
    #     ax5.imshow(img)    
    # ax5.set_xlabel(x_axis_label, fontsize=label_font_size)  # Swap labels
...
##############################################----------------Main()----------------------------################################################
###_____ for Y-axis
with open("D:/results/fmri/2024-01-05_sub-sidtest_ses-001_task-bar_run-01_hemi-L_estimates_[scalable]-test_[grid-151x151x16].json", 'r') as method1_file:
    gem_data = json.load(method1_file)



###_____ for X-axis
with open("D:/results/fmri/2024-01-04_sub-sidtest_ses-001_task-bar_run-01_hemi-L_estimates_[changed-orthonormalization]-[changed-sigma-sign]-[use_np.linalg.solve]-test_[grid-51x51x16].json", 'r') as method2_file:
            oprf_data = json.load(method2_file)

## masking
threshold  = 0.1
r2_mask = [entry['R2'] is not None and not math.isnan(entry['R2']) and entry['R2'] > threshold for entry in gem_data]

# gem Data
gem_centerx0 = [entry['Centerx0'] for i, entry in enumerate(gem_data) if r2_mask[i]]
gem_centery0 = [entry['Centery0'] for i, entry in enumerate(gem_data) if r2_mask[i]]
gem_sigma = [entry['sigmaMajor'] for i, entry in enumerate(gem_data) if r2_mask[i]]
gem_r2 = [entry['R2'] for i, entry in enumerate(gem_data) if r2_mask[i]]
...
plot_title = "Major change"
plot_all(plot_title, gem_centerx0, oprf_centery0
        , gem_centery0, oprf_centerx0
        , gem_sigma, oprf_sigma
        , gem_r2, oprf_r2
        , Result_image_path = f"")
