...
def get_y_hat(voxel_signal, model_signal):
    ...
    # Add cosine and sine regressors
    time_points = len(voxel_signal)
    t = np.arange(0, time_points, 1)
    cosine_regressor = np.cos(2 * np.pi * 1.17 * t)
    sine_regressor = np.sin(2 * np.pi * 0.2 * t)
    
    intercept = np.ones((model_signal.size, 1))    
    #trends = np.vstack([np.linspace(0, 1, len(voxel_signal)) ** i for i in range(8)]).T
    X = np.hstack((model_signal, intercept, cosine_regressor[:, np.newaxis], sine_regressor[:, np.newaxis]))
