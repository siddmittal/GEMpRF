...
def plot_best_matched_responses(matched_indices_array, measured_voxel_responses, modelled_responses):
    ...
    for r2, voxel_idx, match_idx, mse in top_min_mse_values:   
        voxel_signal = measured_voxel_responses[int(voxel_idx)]
        model_signal = modelled_responses[int(match_idx)]
        y_hat, betas, trends = get_y_hat(voxel_signal, model_signal)                   
        
         #plotting
        fig, ax = plt.subplots(2)
        ax[0].plot(voxel_signal, label='y')
        ax[0].plot(y_hat, label='y_hat')

        # Remove linear trend from both voxel_signal and y_hat
        detrended_voxel_signal = voxel_signal - (trends @ betas[1:])
        detrended_y_hat = y_hat - (trends @ betas[1:])

        ax[1].plot(detrended_voxel_signal)
        ax[1].plot(detrended_y_hat)        
        fig.suptitle(f'Best GLM Fitting - R2 = {r2}, Mse = {mse}')

        # Add labels and title
        ax[0].set_xlabel('Time')
        ax[0].set_ylabel('Response')
        ax[0].set_title('Original Signal and y_hat')
        ax[0].legend()

        ax[1].set_xlabel('Time')
        ax[1].set_ylabel('Response')
        ax[1].set_title('Detrended Signals')
        ax[1].legend()
...
def plot_worst_matched_responses(matched_indices_array, measured_voxel_responses, modelled_responses):
    ...
    for r2, voxel_idx, match_idx, mse in worst_min_mse_values:   
        voxel_signal = measured_voxel_responses[int(voxel_idx)]
        model_signal = modelled_responses[int(match_idx)]
        y_hat, betas, trends = get_y_hat(voxel_signal, model_signal)   
        ...
        ax[1].plot(detrended_voxel_signal)
        ax[1].plot(detrended_y_hat)        
        fig.suptitle(f'Worst GLM Fitting - R2 = {r2}, Mse = {mse}')
...
def match_voxel_responses_to_expected_pixel_position_timecourses(measured_voxel_timecourses, expected_responses_grid):
    ...
    for voxel_idx in range(num_voxels):        
        ...
        min_r2 = float('inf')  # Initialize minimum r2 with a high value
        for expected_idx in range(num_expected_responses):
            ...
            y_hat, betas, _ = get_y_hat(observed_voxel_timecourse, expected_response)                       
            mse = np.mean(((observed_voxel_timecourse - y_hat)) ** 2)  # Calculate Mean Squared Error
            # r2 = 1 - ((np.sum((observed_voxel_timecourse - y_hat) ** 2))/np.sum((observed_voxel_timecourse - observed_voxel_timecourse.mean()) ** 2))
            if betas[0]>0:
                r2 = np.sum(np.square(observed_voxel_timecourse - y_hat)) / np.sum(np.square(observed_voxel_timecourse))
            else:
                r2 = 1e17
        ...
        matched_indices.append([min_r2, voxel_idx, best_match_index, min_mse])
...
def get_y_hat(voxel_signal, model_signal):
    ...
    trends = np.vstack([np.linspace(0, 1, len(voxel_signal)) ** i for i in range(nPolynomials)]).T
