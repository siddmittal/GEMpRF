...
sessions = [1, 2, 3, 4, 5]
runs     = [1, 2]
tasks    = ['barStandard', 'barRot', 'barDouble']
...
def get_fits():
    ...
    Sg, Dxg, Dyg, Dsigmag = stim.sampling_tcs[sigma_index]
...
def update_plots(event):
    ...
    vx = int(voxel_slider.val)
    ln1.set_xdata([measurement.data.x[vx]])
    ln1.set_ydata([-measurement.data.y[vx]])
    ax['err'].plot([measurement.data.x[vx]], [-measurement.data.y[vx]], 'x')
    #ax['err'].clabel(CS, CS.levels, inline=True)
    info_txt.set_val(f'Max error: {Eg.max():.5f}\nx: {measurement.data.x[vx]:.2f}/y: {measurement.data.y[vx]:.2f}/sigma: {measurement.data.s[vx]:.2f}\nvar exp: {measurement.data.varexp[vx]:.3f}')
...
voxel_slider.on_changed(update_plots)

subject_rbtn.on_clicked(update_measurement)
session_rbtn.on_clicked(update_measurement)
run_rbtn.on_clicked(update_measurement)
task_rbtn.on_clicked(update_measurement)