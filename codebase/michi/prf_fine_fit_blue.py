...
study = 'stimsim23'
base_dir = os.path.join('/z/fmri/data/', study)
...
ana = PRF.from_docker(study, subject, session, task, run, analysis=prfanalyze_analysis, hemi=hemi)
ana.maskROI('V1')
# ana.maskVarExp(.1)
...
tc = ana.voxelTC
tcs = tc @ stim.R_regress.T

tcs_length_s = np.sum(tcs**2, axis=1)

e_to_rho2_offset = 1 - np.sum(tc**2, axis=1) / tcs_length_s + np.sum((tc @ stim.R)**2, axis=1) / tcs_length_s
e_to_rho2_scale  = 1 / tcs_length_s

S = np.array([stim.signal(xys, return_derivatives=False) for xys in search_space])
grid_fit = S @ tc.T
grid_fit_indices = np.argmax(grid_fit, 0)
del grid_fit

unique_indices, unique_grid_fit_mapping = np.unique(grid_fit_indices, return_inverse=True)

s_and_derivs = [stim.signal(search_space[i]) for i in unique_indices]

hessian_indices = np.array([[0, 1, 2],
                            [1, 3, 4],
                            [2, 3, 5]])
...
for i, grid_fit_mapping in enumerate(unique_grid_fit_mapping):
    s_prime, ds_prime_dm, ds_prime_dm_dl = s_and_derivs[grid_fit_mapping]
    y = tcs[i]
    
    weights.append([1/np.sum(ds_dm**2) for ds_dm in ds_prime_dm])

    de_dm     = np.array([ds_dm    @ y for ds_dm    in ds_prime_dm   ])
    de_dm_dl  = np.array([ds_dk_dl @ y for ds_dk_dl in ds_prime_dm_dl])

    grad = de_dm
    hessian = de_dm_dl[hessian_indices]

    step = np.linalg.solve(hessian, grad)
    steps.append(-step)

    new_pos = search_space[unique_indices[grid_fit_mapping]] - step
    s_new = stim.signal(new_pos, False)
    e_old = s_prime.T @ y
    e_new = s_new.T   @ y

    rho_old = e_old**2 * e_to_rho2_scale[i] + e_to_rho2_offset[i]
    rho_new = e_new**2 * e_to_rho2_scale[i] + e_to_rho2_offset[i]

    old_rho2.append(rho_old)
    new_rho2.append(rho_new)
    
old_rho2 = np.array(old_rho2)
new_rho2 = np.array(new_rho2)
