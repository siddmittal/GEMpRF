...
baseP = "D:\\results\\docker-analysis\\01-test\\BIDS\\derivatives\\prfanalyze-oprf\\sub-001\\ses-20200320"
with open(os.path.join(baseP, "sub-001_ses-20200320_task-prf_acq-normal_run-01_estimates.json")) as f:
    j = json.load(f)


for param in j[0].keys():
    
    param_list = np.array([a[param] for a in j])

    output_file = f"sub-001_ses-20200320_task-prf_acq-normal_run-01_{param}.nii.gz"

    nifti = nib.Nifti1Image(param_list, np.eye(4))
    nib.save(nifti, os.path.join(baseP, output_file))