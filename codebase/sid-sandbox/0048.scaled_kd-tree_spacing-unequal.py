...
def get_scaled_values(input_arr, min_val, max_val):    
    scaled_values = (input_arr - min_val) / (max_val - min_val)
...
scaled_points = np.column_stack((get_scaled_values(X.ravel(), -0.3, 0.3), get_scaled_values(Y.ravel(), -0.3, 0.3), get_scaled_values(Z.ravel(), 1, 6)))
...
scaled_query_point = np.array([([get_scaled_values(0, -0.3, 0.3), get_scaled_values(0, -0.3, 0.3), get_scaled_values(2, 1, 6)])])

# Find the indices of the nearest neighbors and their distances
# distances, indices = kdtree.query(query_point, k=27)
distances, indices = kdtree.query(scaled_query_point, k=27)
