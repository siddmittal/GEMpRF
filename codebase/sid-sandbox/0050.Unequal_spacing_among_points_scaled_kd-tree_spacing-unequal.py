...
def get_unequally_spaced_symmmetric_points(start, end, num_points):
    positive_points = np.linspace(start, end, num_points) + np.cumsum(np.linspace(0, 10 * num_points), num_points)
    negative_points = -np.flip(positive_points)
    points = np.concatenate(negative_points, [0], positive_points)
    return points


# points with UNEQUAL SPACING within the same dimension
x_values = get_unequally_spaced_symmmetric_points(start=0.5, end = 9, num_points=10)
