...
class HGR:
    ...
    def get_parameters(self, n_batch = 10000,
        ...
        # r = np.linspace(0, np.sqrt(2 * max_radius**2), 2000) # <<-----changed from 50 to 2000
        [S, R] = np.meshgrid(s,r)
        S = S.flatten()
        R = R.flatten()
        I = np.zeros(2500)
        # I = np.zeros(2000 * 50)  # <<-----changed from 2500 to 2000*50

        for i in range(2500):
        # for i in range(2000 * 50): # <<-----changed from 2500 to 2000*50
