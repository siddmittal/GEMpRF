...
class OLS:
    def __init__(self
                 , modelled_signals_data : List[ReceptiveFieldResponse]
                 , observations_data : List[ReceptiveFieldResponse]
                 , type = 'ordinary least squares - simplified'):
        self.modelled_data = modelled_signals_data
        self.data_to_be_matched = observations_data
        self.N = modelled_signals_data[0].timecourse.shape[0] # number of data points in each timecourse
    ...
    def compute_proj_squared(self):   
        ...
        self._compute_orthonormal_model_signals(R=R)

        # grid of orthonormal modelled signals
        grid_s_prime = np.vstack([response.timecourse for response in self.orthonormal_modelled_signals]).T

        # grid of measured/simulated target signals
        grid_y  = np.vstack([response.timecourse for response in self.data_to_be_matched]).T
...
def run_grid_fit_simulation_program():
    ...
    # Data matching
    ols = OLS(modelled_signals_data=grid.data
                               , observations_data= data_synthesizer.data)    
    ...
    for i in range(len(best_fit_proj)):
        target_idx = i
        signal_idx = best_fit_proj[i]
        target_synthesizer_location = (noisy_synthetic_data[target_idx].x, noisy_synthetic_data[target_idx].y)
        matched_signal_location = (grid.data[signal_idx].x, grid.data[signal_idx].y)
    ...
    #plt.imshow(grid_fitting_results, cmap='YlGnBu', origin='lower', extent=[0, nCols_grid, 0, nRows_grid], vmin=vmin, vmax=vmax)
