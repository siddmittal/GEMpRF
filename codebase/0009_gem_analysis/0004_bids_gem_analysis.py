...
def main(prf_spatial_points = None):
    ...
    if selected_prf_model == SelectedPRFModel.Gaussian:      
        # NOTE: PRFAnalysis class will be helpful for the concatenation runs, where you can store the results with different stimulus in corresponding objects (i.e. prf_analysis)      
    ...
    if cfg.bids['@enable'] == "True":
        ...
        measured_data = [data[0] for data in measured_data_info_list]        
    else:
    ...
    for data_idx in range(len(measured_data)):
        # result filepath
        if cfg.bids['@enable'] == "True":
            result_file =  GemBidsHandler.inputpath2resultpath(cfg.bids, measured_data_info_list[data_idx])
        else:
