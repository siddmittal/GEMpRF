		...
		if (row < nGaussianRows && col < nGaussianCols)
		{
			...
			for (int stim_vf_row = 0; stim_vf_row < nStimulusRows; stim_vf_row++)
			{
				for (int stim_vf_col = 0; stim_vf_col < nStimulusCols; stim_vf_col++)
				{
					...
					// result_gaussian_curves[gaussIdx] = gaussianModel(x, y, x_mean, y_mean, sigma); //exp(exponent); // / (2 * PI * sigma * sigma);				
					//printf("Thread: (%d, %d), Ux: %f, Uy: %f, result_gaussian_curves[%d]: %.6e\n", col, row, x_mean, y_mean, gaussIdx, result_gaussian_curves[gaussIdx]);

					// Derivatives
					//result_Dx_gaussian_curves[gaussIdx] = ((x - x_mean) / (sigma * sigma)) * exp(-((x - x_mean) * (x - x_mean) + (y - y_mean) * (y - y_mean)) / (2 * sigma * sigma));
					//result_Dy_gaussian_curves[gaussIdx] = ((y - y_mean) / (sigma * sigma)) * exp(-((x - x_mean) * (x - x_mean) + (y - y_mean) * (y - y_mean)) / (2 * sigma * sigma));
					/*result_Dsigma_gaussian_curves[gaussIdx] = -((x - x_mean) ^ 2 + (y - y_mean) ^ 2) / (sigma ^ 3) * exp(-((x - x_mean) ^ 2 + (y - y_mean) ^ 2) / (2 * sigma ^ 2));*/
