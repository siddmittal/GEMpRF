...
test_space_rows = 3
test_space_cols = 3
search_space_frames = 3 #<-----------when we have variable SIGMA
stim_width = 101
stim_height = 101
sigma = np.double(2) ## 2
...
def gpu_compute():
    ...
    test_flat_gaussian_curves_data_gpu = cp.zeros((test_space_rows * test_space_cols * stim_width * stim_height), dtype=cp.float64)
    test_dummy1_flat_gaussian_curves_data_gpu = cp.zeros((test_space_rows * test_space_cols * stim_width * stim_height), dtype=cp.float64)
    test_dummy2_flat_gaussian_curves_data_gpu = cp.zeros((test_space_rows * test_space_cols * stim_width * stim_height), dtype=cp.float64)
    test_dummy3_flat_gaussian_curves_data_gpu = cp.zeros((test_space_rows * test_space_cols * stim_width * stim_height), dtype=cp.float64)
    ...
    gaussian_kernel(grid_dim_1, block_dim_1, (
        ...
        test_dummy1_flat_gaussian_curves_data_gpu,
        test_dummy2_flat_gaussian_curves_data_gpu,
        test_dummy3_flat_gaussian_curves_data_gpu,
        test_space_xx_gpu, 
        test_space_yy_gpu,
        stim_xx_gpu,
        stim_yy_gpu,
    ...
    # test space
    # reshape gpu gaussian curuves row-wise
    test_nRows_gaussian_curves_matrix = test_space_rows * test_space_cols
    ...
    # Original Sid - 16.10.2023 - Wrong gradients problem
    # dS_prime_dx_columnmajor_gpu = dS_star_dx_columnmajor_gpu * S_star_S_star_invroot_gpu -  (S_star_columnmajor_gpu * (S_star_S_star_invroot_gpu ** 3)) @ ((S_star_columnmajor_gpu.T @ dS_star_dx_columnmajor_gpu))
    # dS_prime_dy_columnmajor_gpu = dS_star_dy_columnmajor_gpu * S_star_S_star_invroot_gpu -  (S_star_columnmajor_gpu * (S_star_S_star_invroot_gpu ** 3)) @ ((S_star_columnmajor_gpu.T @ dS_star_dy_columnmajor_gpu))
    ...
    for y_idx in range(Y_signals_gpu.shape[1]):
        ...
        actual_X1 = [(mu_X_grid[a[0], a[1]], mu_Y_grid[a[0], a[1]]) for a in block_2d_indices.astype(int)]
        xx, yy = zip(*actual_X1)
