...
class QuadrilateralSignalsSpace:
    ...
    def _generate_meshgrids(self):
        ...
        gaussian_curve_x_values = np.linspace( - self.stimulus.size_in_degrees, + self.stimulus.size_in_degrees, self.stimulus.data.shape[0])
        gaussian_curve_y_values = np.linspace( - self.stimulus.size_in_degrees, + self.stimulus.size_in_degrees, self.stimulus.data.shape[1])
    ...
    def generate_model_responses(self):
        ...
        for row in range(self.grid_nRows):
            for col in range(self.grid_nCols):
                Z = self._generate_2d_gaussian(mean_x= self.custom_space_meshgrid_X[row][col], mean_y= self.custom_space_meshgrid_Y[row][col])
...
class Histograms:
    def __init__(self, search_space, nRows_search_space, nCols_search_space):     
        ...
        self.ax3d.set_title(f"Histogram for Location (0, 0)")
        ...
        self.surface = self.ax3d.plot_surface(self.search_space.custom_space_meshgrid_Y, self.search_space.custom_space_meshgrid_X, dummy_hist_data, cmap='viridis')

        # Create a white 2D array for the 10x10 square with grid lines
        self.square_data = np.ones((10, 10), dtype=int) * 255  # White color (255 in grayscale)

        # Plot the 2D square in the 2D subplot with grid lines
        self.ax2d.imshow(self.square_data, cmap='gray', extent=[0, 10, 0, 10], vmin=0, vmax=255, origin='lower', interpolation='none')

        # Set the x and y axis ticks and labels between -9 to 9 for the legends
        self.ax2d.set_xticks(range(-9, 10))
        self.ax2d.set_yticks(range(-9, 10))
    ...
    def on_click(self, event):
        if event.inaxes == self.ax2d:  # Check if the click event is on the 2D subplot
            ...
            if 0 <= index < len(self.histograms):
                ...
                self.surface.remove()  # Remove the previous surface plot
                self.surface = self.ax3d.plot_surface(self.search_space.custom_space_meshgrid_Y, self.search_space.custom_space_meshgrid_X, hist_data, cmap='viridis')
                self.ax3d.set_title(f"Histogram for Location ({col}, {row})")
                self.fig.canvas.draw()  # Redraw the figure

    # def plot_histograms(self):
    #     # Plot the 3D surface
    #     hist_data = self.histograms[0].HistData
    #     self.ax3d.plot_surface(self.search_space.custom_space_meshgrid_Y, self.search_space.custom_space_meshgrid_X, hist_data, cmap='viridis')
...
def RunProgram():
    ...
    histograms = Histograms(search_space=search_space
                            , nRows_search_space=nRows_test_space
                            , nCols_search_space=nCols_search_space)
    histograms.compute_histograms(search_space = search_space
