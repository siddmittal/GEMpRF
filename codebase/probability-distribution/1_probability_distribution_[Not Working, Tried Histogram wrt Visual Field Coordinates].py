...
def RunProgram():
    ...
    compute_histograms(search_space = search_space, noisy_test_data=data_synthesizer.data
                       , best_fit_info_wrt_search_space=best_fit_info, 
                       nRows_search_space=nCols_test_space, 
                       nCols_search_space=nCols_search_space, 
                       synthesis_ratio=synthesis_ratio)
                          
    print("done!")

def compute_histograms(search_space : QuadrilateralSignalsSpace
    ...
    for row in range(nRows_search_space):
        for col in range(nCols_search_space):
            ...
            flat_idx = synthesis_ratio*(row*nCols_search_space + col)
            ...
            for i in range(len(relevant_elements_for_histogram)):
                ...
                row_idx = int(best_match_idx_wrt_search_space/nRows_search_space) 
                col_idx = best_match_idx_wrt_search_space - (row_idx * nCols_search_space) 
            ...
            hist2d, _, _ = np.histogram2d(row_coordinates, col_coordinates, bins=(nRows_search_space, nCols_search_space))
            row_edges = search_space.custom_space_row_points
            col_edges = search_space.custom_space_col_points
            histogram.HistData = [hist2d, row_edges, col_edges]
