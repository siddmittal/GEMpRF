...
class Histograms:
    def __init__(self, search_space, nRows_search_space, nCols_search_space):     
        ...
        # Plotting related
        self.fig = plt.figure()
        self.ax = self.fig.add_subplot(111, projection='3d')  # Create a 3D axes object
        self.ax.set_title(f"Histogram for Location (0, 0)")

        # Initialize the initial plot (0, 0)
        dummy_hist_data = np.random.rand(nRows_search_space, nCols_search_space)
        self.surface = self.ax.plot_surface(self.search_space.custom_space_meshgrid_Y, self.search_space.custom_space_meshgrid_X, dummy_hist_data , cmap='viridis')
    ...
    def on_click(self, event):
        if event.inaxes:
            ...
            if 0 <= index < len(self.histograms):
                ...
                X, Y = np.meshgrid(self.search_space.custom_space_meshgrid_Y, self.search_space.custom_space_meshgrid_X)
                self.surface.remove()  # Remove the previous surface plot
                self.surface = self.ax.plot_surface(X, Y, hist_data, cmap='viridis')
                self.ax.set_title(f"Histogram for Location ({col}, {row})")
    ...
    def plot_histograms(self):
        ...
        ax.plot_surface(self.search_space.custom_space_meshgrid_Y, self.search_space.custom_space_meshgrid_X, hist_data, cmap='viridis')

        ax.set_title(f"Histogram for Location (0, 0)")
        plt.show()
        fig.canvas.mpl_connect('button_press_event', self.on_click)  

    def compute_histograms(self, search_space : QuadrilateralSignalsSpace
        ...
        for row in range(self.nRows_search_space):
            for col in range(self.nCols_search_space):
                ...
                for i in range(len(relevant_elements_for_histogram)):
                    ...
                    row_coordinates.append(row_idx)
                    col_coordinates.append(col_idx)
